...
if (isset($_POST['insert'])) {
  ...
  // Insert the values in the user2category table
  if ($OK && isset($_SESSION['username'])) {
  	$article_id = $stmt->insert_id;
  	$sql = 'INSERT INTO user2article (user_id, article_id)
  			VALUES(?, ?)';
  	$stmt->prepare($sql);
  ...
  if ($OK && !isset($imageError) && !isset($catError)) {
	header('Location: http://oneshot.gr/admin/blog_list_mysqli.php' . SID);
	exit();
  } else {
	$error = $stmt->error;
	if (isset($imageError)) {
